...
| **P2a** | Egress / redaction / data-governance gate | **✅ v1 done (2026-06-01: allowlist redaction live; in-region client = follow-up).** **Widened 2026-09-14, shipped 2026-09-14** ([ADR 0007 amendment](decisions/0007-egress-data-governance.md#amendment-2026-09-14--the-operators-question-is-now-in-scope-pii-coverage-widens-and-a-real-enforcement-gap-was-found)) — a same-day audit found the widening had been *decided* but not yet coded (the operator's question, the PII widening, and the `claude_client.py` backstop were all still open); all three are now implemented: `agent.py`'s `_build_user_message`/`_traced_chat` redact the operator's question before either the LLM prompt or the Langfuse trace sees it, `redact.go`/both `log_redaction.py` copies gained phone/IPv4/IPv6/GitHub/Slack/Google/Stripe/generic-`sk-` patterns plus a Luhn-validated credit-card pass, and `config/claude_client.py`'s Anthropic client is now wrapped with a redaction backstop. `policies/data-governance.md` updated to match (the operator's-question item is no longer listed under Non-goals). The real enforcement gap found in the same pass (`incident_embeddings.summary` stored unredacted) is tracked separately as **OPS-13**, fixed the same day | [ADR 0007](decisions/0007-egress-data-governance.md) + [policy](policies/data-governance.md) |
...
2. **Every section carries its own freshness and coverage**, because a
   ...
   document is, so dropping the word "document" does not relax this.
   *Coverage shipped* with per-node figures (`d15e466`) and the
   incomplete-graph banner (`bc9771b`). **Freshness shipped 2026-09-05**, and
   the gap it closed is worth recording, because it survived four rounds of
   "make the diagram more informative" unnoticed: the *edges* had carried a
   staleness signal since they shipped (dashed past 15 minutes, plus a "Stale
   edges" stat), while the node boxes drew the fastest-moving data on the
   diagram — replicas ready, restarts, pod phase — with no way to tell 30
   seconds from three hours. Adding information to a box is visible work;
   noticing that the box cannot say when it was last true is not. See P21 for
   the half still owed.
...
### Phase 2 — outcome and port (pure parsing wins, biggest payoff) — **SHIPPED 2026-09-12** ([ADR 0031](decisions/0031-edge-outcome-and-port-capture.md))

- **Outcome counters** — success / failure / timeout captured per edge
  (retry state is not part of this phase — see phase 7's circuit-breaker
  item, proposed the same day). This converts a topology graph into a
  **health** graph: "is this dependency working" is a more useful question
  than "does it exist".
...
outcome counters, accumulated the same way `evidence_count` already is.
`extract_service_mentions` itself was left untouched (pinned by ~26 tests)
