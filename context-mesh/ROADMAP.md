...
| OPS-8 | **payments-test manifests reference Docker Hub in a namespace with no internet route** | `payment-service/worker/api.yaml` use plain `alpine:3.18` / `alpine:3.19` / `nginx:1.25-alpine`, but that namespace's Fargate profile has **no NAT** — only VPC endpoints for ecr.api / ecr.dkr / kinesis-firehose (ADR 0021) — so Docker Hub is unreachable. `03-vault-bootstrap.yml` rewrites `ACCOUNT_ID.dkr.ecr…` to the real registry, and **no manifest contains that placeholder**, so the `sed` is a no-op. `scripts/mirror_base_images.sh` already mirrors every needed tag to `agentify/log-test-base/*`. **This is the leading candidate for OPS-5**: ImagePullBackOff presents as phase `Pending`, which is exactly what the agent reported for `payment-worker` and `payment-api`. Confirm with `kubectl describe pod -n payments <pod> \| tail -20`; if it is image pulls, point the three manifests at the mirrored tags (as `payment-batch.yaml` now does). | 2026-09-01 |
